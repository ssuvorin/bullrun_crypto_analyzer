...
// Helper functions (moved from gemini-ai.js)
...
function parseAIResponse(data, cryptoData = null) {
  try {
    ...
    // Trying to find JSON in the response
    let jsonMatch = text.match(/\{[\s\S]*\}/);
    
    if (!jsonMatch) {
      // If JSON not found, try to find between ```json and ```
    ...
    // Clean JSON from possible issues
    jsonStr = jsonStr
      .replace(/,\s*}/g, '}')
      .replace(/,\s*]/g, ']')
      .replace(/\n/g, ' ')
